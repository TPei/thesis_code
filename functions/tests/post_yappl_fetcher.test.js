const fs = require('fs');
const transform = require('../cloudant/fetch_yappl_for_user/post.js').handler

// test that all request.json / output.json files match
test('handle cloudant query post does its job', () => {
  let input = '../cloudant/fetch_yappl_for_user/post_request.json'
  let output = '../cloudant/fetch_yappl_for_user/post_output.json'
